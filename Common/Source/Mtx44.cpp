...
void Mtx44::SetToRotation(float degrees, float axisX, float axisY, float axisZ) throw( DivideByZero ) 
{
	...
	double c = cos(Math::DegreeToRadian(degrees)), s = sin(Math::DegreeToRadian(degrees));
...
Mtx44 Mtx44::GetTransformation(const Vector3 & Txyz, float Sxyz)
{
	Mtx44 T, R, S;

	T.SetToTranslation(Txyz.x, Txyz.y, Txyz.z);
	//R.SetToIdentity();
	S.SetToScale(Sxyz, Sxyz, Sxyz);

	return T * S;
}

Mtx44 Mtx44::GetTransformation(float Tx, float Ty, float Tz, float Sx, float Sy, float Sz, float Rx, float Ry, float Rz)
{
	Mtx44 T, R, S;
...
void Mtx44::SetToTransform(float Tx, float Ty, float Tz, float Sx, float Sy, float Sz, float Rx, float Ry, float Rz)
{
	*this = GetTransformation(Tx, Ty, Tz, Sx, Sy, Sz, Rx, Ry, Rz);
}
